...
purpose of this note is to make the sensitivity vivid in a sharp form:
...
is consistent.
\end{enumerate}
Consequently, the implications $\Con(\PA)\to\Con_{T_i}(\PA)$ generate all
provable implications among the sentences
$\Con(\PA),\Con_{T_1}(\PA),\dots,\Con_{T_n}(\PA)$, and the realizable
verdict patterns in models of $\PA$ are exactly: all verdicts true, or
$\Con(\PA)$ false and the remaining verdicts arbitrary.
\end{theorem}
...
The proof is a Rosser argument, but with a twist we found illuminating.
...
rather than assumes, the independence conditions posited in an earlier
sketch of this result (see Remark~\ref{rem:dynamics}). The technique
...
verification artifact is available from the author.
...
Conversely, suppose $\neg\Con(\PA)$ together with the displayed pattern of
verdicts. By $\neg\Con(\PA)$ and the least number principle there is a
least inconsistency proof $p$. For $i\in S$: $\Con_{T_i}(\PA)$ applied to
$p$ yields $\HT{T_i}(p)$. For $i\notin S$: $\neg\Con_{T_i}(\PA)$ provides
some $y$ with $\Pf(y,\godel{0{=}1})\wedge\neg\HT{T_i}(y)$; minimality
gives $p\le y$, and monotonicity (contrapositively) gives
...
The two descriptions are intertranslatable; we find the dynamical one
the more vivid and the syntactic one the more economical to verify.
\end{remark}

\begin{remark}[Nonstandard dynamics; independence discharged]
\label{rem:dynamics}
An earlier sketch of this result posited, as hypotheses, the mutual
independence of the halting statements of the machines and their joint
independence from $\Con(\PA)$, and then faced the problem---correctly
identified in that sketch---that Boolean independence cannot by itself
control the \emph{relative position} of nonstandard witnesses: one needs
$T_i$ to halt \emph{below} the least inconsistency proof, and no
combination of independence hypotheses delivers an order-type. The
construction above dissolves the problem rather than solving it: the
machines are not chosen independent, they are chosen self-referential, so
that any proof constraining the verdict pattern defeats itself, and the
positional control is exactly what the pattern sentences $\sigma_S$
assert.

It is worth pausing over what the machines actually do inside a model
...
proof-theoretic hierarchy. It would be interesting to locate the
...
consistency. The situation is more delicate and, we think, more
interesting. The key is to make explicit the Rosser-style race that has
...
$\PA\vdash\rho\to\neg\Con(\PA)$ seems especially delicate: there,
...
right statement of the infinitary classification seems delicate, since
$\sigma_S$ becomes infinitary.
\end{question}

\begin{question}\label{q:iterate}
The map $T\mapsto\Con_T(\PA)$ invites iteration: one may form
$\Con_T(\PA+\Con_{T'}(\PA))$ and so on. What hierarchy results, under
provability or interpretability, and how does it interact with the
classical progressions of iterated consistency?
\end{question}
...
varying the enumeration of axioms, and to the participants in the
MathOverflow discussion where an early version of the question was
posed. The formal verification reported in the introduction was carried
out with Aristotle (Harmonic), over the Foundation library of the
Formalized Formal Logic project. % [add MO link; add grant info if any;
% repository link for the Lean artifact]
...
% ============================================================
% DRAFT NOTES (not for publication) --- updated for draft 4
%
% -2. NEW IN DRAFT 4: mini round 3 landed and validated. Necessitation
%    (`necessitation`) and extensional identity over N (`provBefore_nat_iff`)
%    are now verified, so the intro's clause-(1) scoping was strengthened
%    and the pending-comment deleted; Prop 6.4 (stage-formula form,
%    `haltR_rhoStage_iff`) joined the verified list (intro + Remark on
%    pacing). Raw #print axioms archived in the artifact (AXIOMS.txt);
%    BUILD.md now documents the pinned triple and restore procedure.
%    HONEST CAVEAT, reflected in the intro paragraph: the acceptance test
%    `main (T := PA)` additionally depends on Foundation's
%    `PA_delta1Definable` --- a literal `axiom` in the library stubbing
%    the standard fact that PA is Delta_1-axiomatized (independently
%    confirmed in Foundation source at the pin, Examples.lean:17). The
%    generic development (all fourteen named results) is free of it.
%    If Foundation later proves the instance, delete the caveat clause.
%    REMAINING before submission: repository link (intro + acks), MO link.
%
% -1. NEW IN DRAFT 3: implemented edits E1-E7 from the round-2 validation
%    memo, following full Lean verification of Thm 4.1, Cor 4.2, Prop 6.2:
%      E1 Remark 4.3 (syntactic reading / simultaneous Rosser fixed point)
%      E2 tie-break in Conv 2.1 + proof of Thm 4.1 (unique conclusions
%         now a convenience, not a load-bearing assumption)
%      E3 Remark 6.5 (pacing artifact) + restated Obstruction B
%      E4 scope footnote on "assuming only Con(PA)" (clauses (1)-(3)
%         machine-verified from Con alone; final clause needs PA |/- ~Con)
%      E5 Remark base-theory upgraded to verified claim (Delta_1-axiomatized
%         r.e. extensions of ISigma1)
%      E6 eliminability note in Conv 2.1
%      E7 formalization paragraph in intro + Aristotle/Foundation acks
%    PENDING before submission: repository link (two places); Aristotle
%    mini round 3 (necessitation corollary, extensional-identity corollary,
%    PA-instantiation example, committed #print axioms outputs); MO link.
%    If mini round 3 lands, delete the pending-comment in the intro
%    paragraph and consider strengthening the intro's clause-(1) scoping.
%
% 0. NEW IN DRAFT 2 (single-author; JDH thanked in acks):
%    Section 6 replaces the old naive Question ("is every Pi_1 consequence
%    of Con a Con_T?"). The naive guess appears to be WRONG, or at least
%    much subtler, and the section records what an evening's push showed:
%    (a) PROVED normal form: Con_T <-> Con v Halt^R(T), for ALL T (the
%        non-halting hypothesis is only needed for extensional
%        correctness, not for the equivalence). Proof is 6 lines; checked
%        both directions twice. Note the pleasant reading: "either
%        consistency, or the machine beat the inconsistency to the tape."
%    (b) PROVED realization: every Sigma_1 rho false in N contributes its
%        clock-paced Rosser truncation rho^{R,c} to the class H. Note the
%        truncation point is the machine CLOCK c(x), not the witness x
%        itself --- the simulation overhead is ineliminable and this is
%        exactly why (B) below is open rather than trivial. An earlier
%        attempt truncated at the witness value; the two versions are
%        provably inequivalent in general. Do not "simplify" this.
%    (c) The classification reduces (Cor 6.5) to Obstructions (A) and (B),
%        both posed as questions. Current beliefs: (A) fails (slow
%        consistency the candidate; end-extension strategy sketched in
%        the question); (B) fails (the rho -> ~Con case looks fatal:
%        Halt^R(T) sees no inconsistency proof below the halt, yet would
%        have to certify inconsistency above it; smells like it needs a
%        reflection-like principle PA does not prove --- BUT this is a
%        smell, not a proof; a clever speeded-up-proof-system machine
%        might yet do it. Guaspari--Solovay / Lindstrom Ch. on Sigma_1
%        sentences and witness comparison is where to look).
%    Sanity checks run on (a): T = never-halting trivial machine gives
%    Halt^R = false, Con_T = Con (correct); T = halt-at-step-0 machine
%    gives Halt^R = true, Con_T = provable (correct: every proof of 0=1
%    has H_T). These cost nothing and catch orientation errors.
%
% 1. [RESOLVED in draft 1] A tempting "independence for free" corollary
%    (all Boolean patterns of Halt(T_i) vs ~Con realized) turned out to be
%    subtler than it looks: Halt(T_i) provably implies only "some watched
%    sentence is provable," not ~Con(PA); and sigma_S controls halting BY
%    stage p, not eventual halting. Replaced by Remark on nonstandard
%    dynamics, which makes the (correct and more interesting) point that
%    in models of ~Con the machines really do trigger, at nonstandard
%    stages. If we want a precise independence statement later, it needs
%    its own argument; note models of Con(PA) can still prove some
%    ~sigma_S internally... no wait, PA |/- ~sigma_S absolutely, but a
%    MODEL of Con can satisfy Pr(~sigma_S) with a nonstandard proof. So
%    even in Con-models the internal trigger may fire. Nice. Discuss.
%
% 2. Decide: state main theorem for PA only (current) or push to EA/S^1_2.
%
% 3. Question 6.6 formulation is a placeholder; sharpen or cut.
%
% 4. Add MathOverflow provenance in acknowledgments + link.
%
% 5. Abstract currently promises "recursion theorem" --- confirm we want
%    the controller-index phrasing in Section 4 or the n-fold statement.
% ============================================================
